    ...
    def DiscountedReturn(self, occlusions, predatorHome, knowledge,
        ...
        for depth in ExperimentParams.Depth:
            ...
            if depth == 5000:
                ...
                # TODO - I think we need to add a mkdir call here, since summaryFile uses `Predator_x` and this may not exist yet
                # note that i also think we enter here instantly, since depth is definitely 5000
                # print("made it to the area in question, is this a problem?")
                # fix for the non-existant directory 
                Path(summaryFile).parent.mkdir(parents=True, exist_ok=True)
                df.to_csv(summaryFile, encoding='utf-8', index=False)
            # print("if not a problem, this should print")
